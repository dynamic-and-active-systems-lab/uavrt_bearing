function [pulses, commands] = readpulsecsvtable(filepath)
...
if fid == -1
    ...
    %pulses = PulseStruct(tagID, freq_MHz, lat, lon, pos_AGL_m, roll_deg, pitch_deg, yaw_deg, time, time_next, snrdB, stftMag2, groupSeqCount, groupIndex , groupsnrdB, noisePSD,detectStatus , confirmStatus, ant_off);
    pulses = table(tagID, freq_MHz, lat, lon, pos_AGL_m, pos_ASL_m, roll_deg, pitch_deg, yaw_deg, time, time_next, snrdB, stftMag2, groupSeqCount, groupIndex , groupsnrdB, noisePSD,detectStatus , confirmStatus, ant_off);
    ...
    commands = table(commandIDs, commandLats, commandLons,commandAltASL, commandAltAGL,'VariableNames',{'id','lat','lon','altAGL','altASL'});
